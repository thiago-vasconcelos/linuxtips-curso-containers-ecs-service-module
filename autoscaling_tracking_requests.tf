resource "aws_appautoscaling_policy" "target_tracking_requests" {
  ...
  name = format("%s-%s-requests-traking", var.cluster_name, var.service_name)
  ...
  target_tracking_scaling_policy_configuration {
    ...
    predefined_metric_specification {
      ...
      resource_label         = format("${data.aws_alb.main[count.index].arn_suffix}/${aws_alb_target_group.main[count.index].arn_suffix}")
    }
  }
}